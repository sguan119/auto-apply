"""autoapply.core.deliver.submit —— READY_TO_SUBMIT → SUBMITTING → CONFIRMING（决策六终态 / PRD 八）。
...
包（`src/autoapply/cli/`）的实现细节，`core` 不能反向依赖 `cli`（CLAUDE.md「核心逻辑与
...
from autoapply.core.contracts import JobRef, Question
